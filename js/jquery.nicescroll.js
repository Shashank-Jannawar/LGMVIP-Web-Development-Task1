    ...
    var NiceScrollClass = function(myopt, me) {
        ...
        this.delayed = function(name, fn, tm, lazy) {
            ...
            if (dd && dd.last + tm > nw && !dd.tt) {
                self.delaylist[name] = {
                    last: nw + tm,
                    tt: setTimeout(function() { if (self || false) { self.delaylist[name].tt = 0;
                            fn.call() } }, tm)
                }
            } else if (!dd || !dd.tt) {
                self.delaylist[name] = {
        ...
        this.debounced = function(name, fn, tm) {
            ...
            if (!dd) {
                setTimeout(function() { var fn = self.delaylist[name];
...
*/
            ...
            if (name == 'mousewheel') {
                ...
                if (cap.cantouch && /mouseup|mousedown|mousemove/.test(name)) { // touch device support
                    ...
                    self._bind(el, tt, function(e) {
                        if (e.touches) {
                            if (e.touches.length < 2) { var ev = (e.touches.length) ? e.touches[0] : e;
                                ev.original = e;
                                fn.call(this, ev); }
                        } else if (e.changedTouches) { var ev = e.changedTouches[0];
                            ev.original = e;
                            fn.call(this, ev); } //blackberry
                    }, bubble || false);
                }
        ...
        function execScrollWheel(e, hr, chkscroll) {
            ...
            if (px) {
                ...
                self.lastdeltax += px;
                self.debounced("mousewheelx", function() { var dt = self.lastdeltax;
                    self.lastdeltax = 0; if (!self.rail.drag) { self.doScrollLeftBy(dt) } }, 15);
            }
            if (py) {
                ...
                if (self.scrollmom) { self.scrollmom.stop() }
                self.lastdeltay += py;
                self.debounced("mousewheely", function() { var dt = self.lastdeltay;
                    self.lastdeltay = 0; if (!self.rail.drag) { self.doScrollBy(dt) } }, 15);
            }
